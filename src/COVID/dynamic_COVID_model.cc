...
dynamic_COVID_model::get_forward_model_function() {
  std::function<Eigen::VectorXd(const Eigen::VectorXd&)> forward_model =
      std::bind(&dynamic_COVID_model::forward_model, this,
...
SparseMD dynamic_COVID_model::eval_transition_probability_matrix(
  ...
  SparseMD loc_transition_matrix = calc_location_transition_matrix(
      parameter_exp, parameter_locations, ensemble_density);
  SparseMD inf_transition_matrix = calc_infection_transition_matrix(
      parameter_exp, parameter_locations, ensemble_density);
  SparseMD cli_transition_matrix =
      calc_clinical_transition_matrix(parameter_exp, parameter_locations);
  SparseMD test_transition_matrix = calc_testing_transition_matrix(
      parameter_exp, parameter_locations, ensemble_density);
